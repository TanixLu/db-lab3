    ...
    <el-col :span="8">
      <div class="pagination-block">
        <el-table :data="info.results" border @row-click="fillInput" size="small">
          ...
          <el-table-column prop="saving_account_id" label="储蓄账户号"/>
        </el-table>
        <el-pagination
    ...
    <el-col :span="9">
      ...
      >
        ...
        <el-form-item>
          ...
          <el-popconfirm
            :title="'确认要删除储蓄账户' + input_fields[3].input_ref + '吗？'"
...
const client_branch_fields = [
  ...
  {'name': 'saving_account_id', 'label': '储蓄账户号'},
]
...
const input_fields = [
  ...
  {'name': 'branch_name', 'label': '支行名'},
  {'name': 'saving_account_id', 'label': '储蓄账户号'},
  {'name': 'currency', 'label': '货币类型'},
  {'name': 'balance', 'label': '余额'},
  {'name': 'interest_rate', 'label': '利率'},
  ...
  methods: {
    ...
    fillInput: function (client_branch) {
      ...
      if(client_branch['saving_account_id']){
        vm.input_fields[3].input_ref = client_branch['saving_account_id']
        axios.get(api_url + 'savingaccount/' + client_branch['saving_account_id'] + '/', auth_config)
          .then(response => {
            vm.input_fields[4].input_ref = response.data['currency']
            vm.input_fields[5].input_ref = response.data['balance']
            vm.input_fields[6].input_ref = response.data['interest_rate']
            vm.input_fields[7].input_ref = response.data['staff_id']
            vm.input_fields[8].input_ref = response.data['open_date']
            vm.input_fields[9].input_ref = response.data['last_access_date']
          })
      }
      else for(let i = 3; i <= 9; i++) vm.input_fields[i].input_ref = ''
    },
    ...
    openSavingAccount: function () {
      ...
      axios.post(api_url + 'client_branch/open_saving_account/', data, auth_config)
          .then(response => {
    ...
    updateSavingAccount: function () {
      ...
      axios.put(api_url + 'savingaccount/' + input_fields[3].input_ref + '/', data, auth_config)
        .then(response => {
    ...
    deleteSavingAccount: function () {
      axios.delete(api_url + 'savingaccount/' + input_fields[3].input_ref + '/', auth_config)
        .then(resopnse => {
